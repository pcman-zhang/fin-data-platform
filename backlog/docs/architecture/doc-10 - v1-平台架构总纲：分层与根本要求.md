...
平台的核心不是"把数据存下来"，而是三份**机读关系**，各自回答一个问题：

| 支柱 | 回答的问题 | 机读形态 | 一致性校验（质量门禁） |
|---|---|---|---|
| **Dictionary** | 我们**有哪些数据**（字段 / 口径 / 单位 / PIT / 血缘 / SLA） | `dictionary/*.yaml` → JSON Schema + DDL | 字典 ↔ DDL 漂移、命名/单位/引用、语义版本规则 |
| **Entity Registry** | **谁是谁**（entity / issuer / listing 的身份与关系、代码履历、外部标识） | `ref.entity*`（SCD2 + 版本 + 知识时间） | 唯一性/区间不重叠、关系词表、多源代码映射、Universe 覆盖 |
| **Derived Engine** | 指标**之间的依赖**（因子图：raw → 因子 → 因子） | 算法注册表 + `derived.inputs`（统一引用语法） | 无环、输入存在、接口一致、上游算法指纹 |

**数据质量不是靠事后扫描"补"出来的，而是这三份关系一致时的涌现结果**：

- 字典一致 ⇒ 字段/口径/单位可被程序验证（避免"看起来对"的数值）；
- 身份一致 ⇒ 代码变更、退市、多源映射不影响时序拼接（避免错配与幸存者偏差）；
- 依赖一致 ⇒ 重算范围、执行顺序、as-of 语义确定（避免混合 vintage 与静默错值）；
- 质量扫描（TASK-4）则在这三份关系之上做**可计算**的检查（唯一/范围/引用/时效/对账），
  而不是靠人工经验。
